# Predicting when Mariano will be substituted w/out conditioning on a sub occuring
...
# read the data in
data = pd.read_csv(GLOBAL_VARS.DATA_SET_PATH + "/mariano_general_sub_home_20091011.csv")
...
# fit the model
logit = sm.Logit(data['mariano_in'], data[predictors])
result = logit.fit()
print result.summary()

#assess its fit
predictions = result.predict(data[predictors])
print util.confusion_matrix_binary_response(data['mariano_in'].tolist(),predictions)
